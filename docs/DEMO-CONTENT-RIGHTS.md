...
Terpwright Phase 3 helpers should preserve this distinction. The current IF Archive preview normalizes paths and URLs, the IFDB preview can show catalog metadata candidates through IFDB's official API, and the IFWiki preview can show reference metadata through IFWiki's MediaWiki API. Preview or lookup results should not be treated as rights clearance, copied wholesale into package docs, or used to auto-download story files, artwork, or historical materials.
