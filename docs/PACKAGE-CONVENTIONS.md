...
The Admin2 Ecosystem Metadata Preview also includes a Metadata Cross-check table. It compares current visible metadata with package-root `metadata.iFiction.xml` when available, IFDB preview values, IFWiki preview values, and normalized IF Archive path/URL values. Supported rows include bibliographic fields, IFIDs, format, catalog URLs, source/upstream/port repository URLs, license name/URL, and tags where those sources provide them. Cross-check apply controls update only the visible Create Package form or metadata editor state; existing package writes still require `Save Metadata`, and draft creation still requires the Create Package submit action. Reference-only and rights-related values must be curator-reviewed and do not prove redistribution permission.
...
- Admin2 can preview curator-supplied ecosystem references, normalize IF Archive path/URL metadata without remote fetches or writes, preview IFDB metadata through IFDB's official API, preview IFWiki metadata through IFWiki's MediaWiki API, and cross-check those candidates against current metadata and package-local iFiction XML where present.
