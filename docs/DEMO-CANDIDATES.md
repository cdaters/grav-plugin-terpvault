...
- Zork II remains a candidate until approval and any explicit `_demo` promotion decision are complete. The 2026-06-02 artifact-basis recheck selected the documented patched source-built historical-header artifact, and a DDEV-only draft package has passed route, story-checksum, manifest, dfrotz, export/import, temporary import route checks, and final package audit. It now has classified feelies, draft helper docs, package-local art, gameplay screenshots, release-specific IFID/catalog fields, and package-local `metadata.iFiction.xml`. A later source/playback repair pass traced the normal-output post-crown balloon crash to a `FANTASIES` table-counter issue; the repaired artifact is now adopted in the DDEV package as SHA-256 `02830587cfe5ca68c2f9289a9178780761ccec5f1582d13130d6217bd9e437ef` and completes under normal dfrotz output without `SUPERBRIEF`, reaching `400/400` in `372` moves with rank `Master Adventurer`. It is ready for Craig approval and a later explicit promotion pass if approved. It is not in `_demo`. Zork III has completed its review path for this development demo tree and is bundled in `_demo`.
...
| [Zork II](demo-candidates/ZORK-II.md) | `https://github.com/historicalsource/zork2.git` rechecked at commit `3da9661098809788a99cef00f00c865c6c204f96` on `master`; no tags/releases observed | Strong candidate basis if MIT source release and redistribution requirements are confirmed; preserve upstream MIT license text and attribution; historical packaging/assets remain classified separately under `docs/DEMO-CONTENT-RIGHTS.md` | DDEV-only draft package now uses the repaired patched source-built historical-header artifact as `zork2.z3`; current DDEV story SHA-256 `02830587cfe5ca68c2f9289a9178780761ccec5f1582d13130d6217bd9e437ef`, size 92414, Release 63 / Serial 860811; previous `10015c715e9226c491bbfe23e448df14e859a0d9f905afc4fe0c18d65d176019` artifact is backed up under `/private/tmp/terpvault-zork2-pre-repair-backup/`; IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` added; adopted source patches are `DREARY-ROOM-FCN` build compatibility and `FANTASIES` table-counter repair; dfrotz route verification completed to `400/400` in `372` moves, rank `Master Adventurer`; manifest, temporary route checks, export/import smoke, temporary import route checks, imported-story dfrotz verification, and final package audit passed | Z-machine DDEV draft package only; no `_demo` package yet | Package-local cover/small-cover/hero art and gameplay screenshots copied in DDEV; seven feelies copied in DDEV and classified as historical reference/preservation material, historical reference/preservation requiring caution, or likely commercial material requiring caution | Original how-to, progressive hints, and verified walkthrough are present in DDEV; walkthrough completes without `SUPERBRIEF` | Candidate only; DDEV package restored to draft; ready for approval/promotion decision | Craig approval and explicit `_demo` promotion pass if approved |
...
- Zork II's 2026-06-02 DDEV-only package now uses the repaired artifact with both the `DREARY-ROOM-FCN` compatibility patch and the `FANTASIES` table-counter repair. Initial DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, art/screenshot routes, release-specific metadata/iFiction, material provenance checks, manifest, temporary published routes, normal-output dfrotz verification to `400/400` in `372` moves, export/import smoke, temporary import routes, imported-story dfrotz verification, and final package audit have passed. Approval remains before any `_demo` promotion.
