...
- Zork II remains a candidate until package/playback verification and package-level provenance review are complete. The 2026-06-02 artifact-basis recheck recommends the documented patched source-built historical-header artifact as the next DDEV package-basis candidate, but no package has been assembled. Zork III has completed its review path for this development demo tree and is bundled in `_demo`.
...
| [Zork II](demo-candidates/ZORK-II.md) | `https://github.com/historicalsource/zork2.git` rechecked at commit `3da9661098809788a99cef00f00c865c6c204f96` on `master`; no tags/releases observed | Strong candidate basis if MIT source release and redistribution requirements are confirmed; preserve upstream MIT license text and attribution; historical packaging/assets remain excluded unless separately reviewed | Not ready; unmodified source build still fails; 2026-06-02 scratch recheck produced Frotz-smoke-tested patched source-built artifacts and confirmed a traceable upstream prebuilt artifact; recommended next basis is the patched source-built historical-header artifact `zork2-release63-serial860811.z3` with SHA-256 `10015c715e9226c491bbfe23e448df14e859a0d9f905afc4fe0c18d65d176019` | Likely Z-machine story file package after DDEV/Parchment/package verification of the selected artifact basis | Craig-created art, screenshots from the selected bundled playable version, and original package-local extras; classify any historical reference/preservation materials under the shared policy | Original helper docs and feelies only unless upstream reusable docs have documented reuse rights | Candidate only; artifact basis recommendation documented, package not assembled | Use the documented patched source-built historical-header artifact as the next DDEV package-basis candidate, then verify TerpVault/Parchment playback before any `_demo` work |
...
- Zork II's 2026-06-02 artifact-basis recheck recommends the scratch-only `DREARY-ROOM-FCN` compatibility patch with historical release/serial reassembly as the next package-basis candidate; package work still requires DDEV/Parchment verification and package-local provenance before any `_demo` promotion.
