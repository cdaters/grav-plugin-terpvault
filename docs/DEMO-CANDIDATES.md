...
- Zork II's 2026-06-02 DDEV-only package now uses the repaired artifact with both the `DREARY-ROOM-FCN` compatibility patch and the `FANTASIES` table-counter repair. Initial DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, art/screenshot routes, release-specific metadata/iFiction, material provenance checks, manifest, temporary published routes, and normal-output dfrotz verification to `400/400` in `372` moves have passed. Export/import, audit, and approval remain before any `_demo` promotion.
