...
- Zork III has stronger scratch build/playback evidence, a DDEV-only candidate package, complete-package export/import smoke coverage, a full-potential walkthrough verified with `dfrotz`, and a documented local IFID tooling blocker, but still needs authoritative IFID/catalog/iFiction resolution, final route-delivery/playback recheck, and final audit before bundled-demo review.
