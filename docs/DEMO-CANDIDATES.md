...
- Zork III has stronger scratch build/playback evidence, a DDEV-only candidate package, complete-package export/import smoke coverage, and a polished draft walkthrough with partial `dfrotz` verification, but still needs full walkthrough verification, IFID/catalog/iFiction decisions, final route-delivery recheck, and final audit before bundled-demo review.
