...
- Zork II remains a candidate until package/playback verification and package-level provenance review are complete. The 2026-06-02 artifact-basis recheck selected the documented patched source-built historical-header artifact, and a DDEV-only draft package has passed route, story-checksum, manifest, and dfrotz smoke checks. It now has classified feelies, draft helper docs, package-local art, gameplay screenshots, release-specific IFID/catalog fields, and package-local `metadata.iFiction.xml`. A later source/playback repair pass traced the normal-output post-crown balloon crash to a scratch-repairable `FANTASIES` table-counter issue; the scratch repaired artifact now gets past the descent without `SUPERBRIEF` and route cleanup has reached `Score: 348` / Cerberus, but the selected DDEV artifact has not been replaced and the full walkthrough still does not complete. Export/import and final audit also remain. It is not in `_demo`. Zork III has completed its review path for this development demo tree and is bundled in `_demo`.
...
| [Zork II](demo-candidates/ZORK-II.md) | `https://github.com/historicalsource/zork2.git` rechecked at commit `3da9661098809788a99cef00f00c865c6c204f96` on `master`; no tags/releases observed | Strong candidate basis if MIT source release and redistribution requirements are confirmed; preserve upstream MIT license text and attribution; historical packaging/assets remain classified separately under `docs/DEMO-CONTENT-RIGHTS.md` | DDEV-only draft package assembled from patched source-built historical-header artifact `zork2-release63-serial860811.z3`; DDEV story target `zork2.z3` has SHA-256 `10015c715e9226c491bbfe23e448df14e859a0d9f905afc4fe0c18d65d176019`, size 92412, Release 63 / Serial 860811; IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` added; route/story/helper-doc/selected-feelie/art/screenshot checks and dfrotz smoke passed; normal-output balloon crash reproduced on the selected artifact, then traced to a scratch-repairable `FANTASIES` table-counter source issue; selected artifact not replaced; repaired-artifact route cleanup now reaches `Score: 348` / Cerberus but remains incomplete around Bank/gnome/demon payment ordering; export/import still pending | Z-machine DDEV draft package only; no `_demo` package yet | Package-local cover/small-cover/hero art and gameplay screenshots copied in DDEV; seven feelies copied in DDEV and classified as historical reference/preservation material, historical reference/preservation requiring caution, or likely commercial material requiring caution | Original how-to and progressive hints drafted; walkthrough route reaches the crown/score 175 on the selected artifact but remains unverified. A scratch repaired-source artifact gets past the post-crown descent without `SUPERBRIEF`, reaches `Score: 348`, then fails from late-game route/payment ordering | Candidate only; DDEV package restored to draft | Continue repaired-artifact route cleanup from the Bank/gnome/demon payment blocker, decide whether to reopen the artifact basis for the `FANTASIES` repair, complete walkthrough verification, then run export/import smoke, final audit, and approval before any `_demo` work |
...
- Zork II's 2026-06-02 DDEV-only package uses the scratch-only `DREARY-ROOM-FCN` compatibility patch with historical release/serial reassembly; initial DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, art/screenshot routes, release-specific metadata/iFiction, and material provenance checks passed. The normal-output post-crown balloon crash is now traced to a scratch-repairable `FANTASIES` table-counter issue, but the selected artifact was not replaced. Repaired-artifact route cleanup has progressed to `Score: 348` / Cerberus and now needs late-game Bank/gnome/demon payment ordering cleanup. Export/import, audit, and approval remain before any `_demo` promotion.
