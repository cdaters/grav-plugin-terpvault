...
| [Open Adventure / Colossal Cave](demo-candidates/OPEN-ADVENTURE.md) | https://gitlab.com/esr/open-adventure · branch `master` · commit `993291a21da44234ae9cf303d0ffc0df19ec3c31` | License review in progress: `COPYING` is BSD 2-Clause and project headers include CC-BY-4.0 metadata; redistribution terms differ by file type and must be reviewed per component | Native terminal build succeeds; `make check` reports `115 tests, 2 failures` (saveresume environment prompts), no committed TerpVault-ready playable artifact yet | Source remains native `advent`/`cheat` + `adventure.yaml`/generated C output; no `.z*`/Glulx artifact selected yet | Keep Open Adventure-specific asset work separate until a playable target and rights posture are confirmed | Player-facing helper docs are deferred until playable strategy is chosen | Research only; build/oracle complete, play strategy pending | Select playable format path (preferred: source-to-parser format), resolve savegame/test caveat, and complete rights + playback proof before any package assembly |
...
- The native C build and test baseline are available locally; `make check` currently reports `115 tests, 2 failures` from environment-specific savegame prompt paths.
...
- For TerpVault/Parchment, a playable path must be selected first (likely source conversion path before any `_demo` package work).
