...
- Zork II remains a candidate until final package audit, approval, and any `_demo` decision are complete. The 2026-06-02 artifact-basis recheck selected the documented patched source-built historical-header artifact, and a DDEV-only draft package has passed route, story-checksum, manifest, dfrotz, export/import, and temporary import route checks. It now has classified feelies, draft helper docs, package-local art, gameplay screenshots, release-specific IFID/catalog fields, and package-local `metadata.iFiction.xml`. A later source/playback repair pass traced the normal-output post-crown balloon crash to a `FANTASIES` table-counter issue; the repaired artifact is now adopted in the DDEV package as SHA-256 `02830587cfe5ca68c2f9289a9178780761ccec5f1582d13130d6217bd9e437ef` and completes under normal dfrotz output without `SUPERBRIEF`, reaching `400/400` in `372` moves with rank `Master Adventurer`. Final audit and approval remain. It is not in `_demo`. Zork III has completed its review path for this development demo tree and is bundled in `_demo`.
...
- Zork II's 2026-06-02 DDEV-only package now uses the repaired artifact with both the `DREARY-ROOM-FCN` compatibility patch and the `FANTASIES` table-counter repair. Initial DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, art/screenshot routes, release-specific metadata/iFiction, material provenance checks, manifest, temporary published routes, normal-output dfrotz verification to `400/400` in `372` moves, export/import smoke, temporary import routes, and imported-story dfrotz verification have passed. Final audit and approval remain before any `_demo` promotion.
