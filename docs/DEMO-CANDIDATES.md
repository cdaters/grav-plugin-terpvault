...
- Zork II's 2026-06-02 DDEV-only package uses the scratch-only `DREARY-ROOM-FCN` compatibility patch with historical release/serial reassembly; initial DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, and material provenance checks passed, but walkthrough transcript verification, metadata, art/screenshots, export/import, audit, and approval remain before any `_demo` promotion.
