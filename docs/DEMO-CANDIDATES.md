...
- Zork II remains a candidate until package/playback verification and package-level provenance review are complete. The 2026-06-02 artifact-basis recheck selected the documented patched source-built historical-header artifact, and a DDEV-only draft package has passed route, story-checksum, manifest, and dfrotz smoke checks. It now has classified feelies, draft helper docs, package-local art, gameplay screenshots, release-specific IFID/catalog fields, and package-local `metadata.iFiction.xml`. A later source/playback repair pass traced the normal-output post-crown balloon crash to a scratch-repairable `FANTASIES` table-counter issue, but the selected DDEV artifact has not been replaced and the full walkthrough still fails after `Score: 215`; export/import and final audit also remain. It is not in `_demo`. Zork III has completed its review path for this development demo tree and is bundled in `_demo`.
...
- Zork II's 2026-06-02 DDEV-only package uses the scratch-only `DREARY-ROOM-FCN` compatibility patch with historical release/serial reassembly; initial DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, art/screenshot routes, release-specific metadata/iFiction, and material provenance checks passed. The normal-output post-crown balloon crash is now traced to a scratch-repairable `FANTASIES` table-counter issue, but the selected artifact was not replaced and the route still needs cleanup after `Score: 215`. Export/import, audit, and approval remain before any `_demo` promotion.
