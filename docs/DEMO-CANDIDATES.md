...
| [Open Adventure / Colossal Cave](demo-candidates/OPEN-ADVENTURE.md) | https://gitlab.com/esr/open-adventure · branch `master` · commit `993291a21da44234ae9cf303d0ffc0df19ec3c31` | License review in progress: `COPYING` is BSD 2-Clause and project headers include CC-BY-4.0 metadata; redistribution terms differ by file type and must be reviewed per component before any final release | Native terminal build remains a caveated oracle; package candidate uses an Inform 6 / Z-machine `.z8` port with story SHA-256 `284f3d08ab9cc692457d1a982acf7dddffcaf8bf0b4aa71cb5d1f33d82865d43`; IFID `74146740-24EA-5383-A8BF-8B239CE36DBE`; import/inspect smoke passed before promotion | Draft TerpVault package at `_demo/data/terpvault/games/open-adventure`; `terpvault.status: draft`; `terpvault.featured: false`; includes `metadata.iFiction.xml` | Package-local cover, small cover, hero, screenshots, and draft map/mockup feelie are included for evaluation; final map/poster art remains replaceable | Original helper docs, walkthrough, provenance, known differences, history/details, map notes, and treasure checklist are included as package-local resources | Draft demo/library candidate; not final/release-ready | Keep draft/non-featured, replace or approve final map/poster art, complete release-level provenance review, and re-run route/checksum checks before any publish decision |
...
- The project is now represented in `_demo` by a draft, non-featured TerpVault package using the Inform 6 / Z-machine `.z8` port.
- For provenance, native `advent` remains an oracle rather than the playable package artifact.
- Do not treat unrelated Adventure ports as interchangeable without license review.
- Record and track: implementation/commit, license scope, chosen playable artifact type, build command, output artifact hash, interpreter/play strategy, final art replacement/approval, and route/checksum validation before any publish decision.
