...
- Open Adventure / Colossal Cave remains a candidate until the exact repository commit, playable artifact strategy, and redistribution posture are verified.
...
| [Zork II](demo-candidates/ZORK-II.md) | `https://github.com/historicalsource/zork2.git` rechecked at commit `3da9661098809788a99cef00f00c865c6c204f96` on `master`; no tags/releases observed | Reviewed for this bundled development demo tree from the verified MIT source release; preserve upstream MIT license text and attribution; historical packaging/assets remain classified separately under `docs/DEMO-CONTENT-RIGHTS.md` | Promoted `_demo` package uses the repaired patched source-built historical-header artifact as `zork2.z3`; story SHA-256 `02830587cfe5ca68c2f9289a9178780761ccec5f1582d13130d6217bd9e437ef`, size 92414, Release 63 / Serial 860811; IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` recorded; adopted source patches are `DREARY-ROOM-FCN` build compatibility and `FANTASIES` table-counter repair; dfrotz route verification completed to `400/400` in `372` moves, rank `Master Adventurer`; manifest, route checks, export/import smoke, final package audit, helper-doc polish, `_demo` copy validation, and clean DDEV seed verification passed | Bundled Z-machine starter package at `_demo/data/terpvault/games/zork-ii` | Package-local cover/small-cover/hero art and gameplay screenshots included; seven feelies classified as historical reference/preservation material, historical reference/preservation requiring caution, or likely commercial material requiring caution | Polished original how-to, progressive hints, and verified walkthrough are present; walkthrough completes without `SUPERBRIEF` | Reviewed bundled demo; included in `_demo`; walkthrough fully verified for the bundled story | Keep package in `_demo`; re-run route/checksum checks if package files or installer behavior change |
...
| [Open Adventure / Colossal Cave](demo-candidates/OPEN-ADVENTURE.md) | https://gitlab.com/esr/open-adventure · branch `master` · commit `993291a21da44234ae9cf303d0ffc0df19ec3c31` | License review in progress: `COPYING` is BSD 2-Clause and project headers include CC-BY-4.0 metadata; redistribution terms differ by file type and must be reviewed per component | Native terminal build succeeds and is useful as a caveated oracle; `make check` reports `115 tests, 2 failures` (`saveresume.1`, `saveresume.3` from save/resume prompt/session behavior), no committed TerpVault-ready playable artifact yet | Preferred future target is a Z-machine `.z8` port, likely via Inform 6 or another documented Z-machine authoring path; source remains native `advent`/`cheat` + `adventure.yaml`/generated C output today; TerpVault cannot play native `advent` directly | Keep Open Adventure-specific asset work separate until a playable target and rights posture are confirmed | Player-facing helper docs are deferred until playable strategy is chosen | Research only; caveated oracle complete, `.z8` play strategy pending | Prove a tiny `.z8`/Z-machine slice outside the TerpVault repo, resolve savegame/test caveat, and complete rights + playback proof before any package assembly |
...
- Zork II's 2026-06-02 package now uses the repaired artifact with both the `DREARY-ROOM-FCN` compatibility patch and the `FANTASIES` table-counter repair. DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, art/screenshot routes, release-specific metadata/iFiction, material provenance checks, manifest, normal-output dfrotz verification to `400/400` in `372` moves, export/import smoke, helper-doc polish, final package audit, `_demo` copy validation, and clean DDEV seed verification have passed. Craig approved it for `_demo` promotion.
...
- Open Adventure is captured as a strong research-oracle candidate, sourced from `https://gitlab.com/esr/open-adventure` (`master`, commit `993291a21da44234ae9cf303d0ffc0df19ec3c31`).
- The native C build and test baseline are available locally; `make check` currently reports `115 tests, 2 failures` from environment-specific savegame prompt paths (`saveresume.1`, `saveresume.3`).
- The project is currently not packaged in TerpVault; current artifacts are native binaries + source (`advent`, `cheat`, `adventure.yaml`, generated `dungeon.c/.h`).
- For TerpVault/Parchment, native `advent` is an oracle rather than a playable package artifact. The preferred playable path is a Z-machine `.z8` port, likely through Inform 6 or another documented Z-machine authoring path, before any `_demo` package work.
- Do not treat unrelated Adventure ports as interchangeable without license review.
- Record and track: implementation/commit, license scope, chosen playable artifact type, build command, output artifact hash, and interpreter/play strategy before any package assembly.
