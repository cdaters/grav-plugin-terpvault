...
| [Zork III](demo-candidates/ZORK-III.md) | `https://github.com/historicalsource/zork3.git` rechecked at commit `3ec9ed412b5f3cafe65d83c727d07db1fe4a86a8` on `master`; no tags/releases observed | Strong candidate basis if MIT source release and redistribution requirements are confirmed; preserve upstream MIT license text and attribution; historical packaging/assets remain excluded unless separately licensed | Source build and `dfrotz` smoke were reverified in scratch on 2026-05-29, and a real DDEV-only candidate package was assembled at `/Users/cdaters/Sites/grav2.0-ddev/user/data/terpvault/games/zork-iii`; story-route diagnosis, complete-package export/import smoke, full walkthrough transcript verification, release-specific IFID/catalog metadata review, final audit, and final route/playback recheck passed on 2026-05-31 | Likely Z-machine story file package; see [candidate package plan](demo-candidates/ZORK-III-PACKAGE-PLAN.md) | Craig-created/original art, screenshots, and package-local feelies were copied into the DDEV-only candidate package; see [asset/materials plan](demo-candidates/ZORK-III-ASSET-PLAN.md) | Original helper docs were refreshed in the DDEV-only package on 2026-05-31; walkthrough is verified with `dfrotz -p -m -s 41` against the exact package story, reaching 7 of 7 in 330 moves; IFID `ZCODE-25-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` are recorded; do not reuse `invisicluesiii.mss` without separate rights review | Ready for Craig approval; DDEV package left draft; not yet copied to `_demo` | Get Craig approval, then run a dedicated `_demo` promotion pass with route/checksum rechecks |
...
- Zork III has stronger scratch build/playback evidence, a DDEV-only candidate package, complete-package export/import smoke coverage, a full-potential walkthrough verified with `dfrotz`, release-specific IFID/catalog/iFiction metadata, and a passed final audit. It is ready for Craig approval before any `_demo` promotion pass.
