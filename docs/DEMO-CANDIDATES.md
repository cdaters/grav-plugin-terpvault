...
- Zork III has stronger scratch build/playback evidence, a DDEV-only candidate package, complete-package export/import smoke coverage, a full-potential walkthrough verified with `dfrotz`, and release-specific IFID/catalog/iFiction metadata, but still needs final route-delivery/playback recheck and final audit before bundled-demo review.
