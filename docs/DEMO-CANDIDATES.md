...
- Zork III has stronger scratch build/playback evidence, a DDEV-only candidate package, complete-package export/import smoke coverage, and a full-potential walkthrough verified with `dfrotz`, but still needs IFID/catalog/iFiction decisions, final route-delivery/playback recheck, and final audit before bundled-demo review.
