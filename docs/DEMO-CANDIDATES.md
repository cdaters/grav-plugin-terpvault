...
| [Zork II](demo-candidates/ZORK-II.md) | `https://github.com/historicalsource/zork2.git` rechecked at commit `3da9661098809788a99cef00f00c865c6c204f96` on `master`; no tags/releases observed | Strong candidate basis if MIT source release and redistribution requirements are confirmed; preserve upstream MIT license text and attribution; historical packaging/assets remain classified separately under `docs/DEMO-CONTENT-RIGHTS.md` | DDEV-only draft package assembled from patched source-built historical-header artifact `zork2-release63-serial860811.z3`; DDEV story target `zork2.z3` has SHA-256 `10015c715e9226c491bbfe23e448df14e859a0d9f905afc4fe0c18d65d176019`, size 92412, Release 63 / Serial 860811; IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` added; route/story/helper-doc/selected-feelie/art/screenshot checks and dfrotz smoke passed; normal-output balloon crash reproduced on the selected artifact, then traced to a scratch-repairable `FANTASIES` table-counter source issue; selected artifact not replaced; repaired scratch artifact now completes under normal dfrotz output to `400/400` in `372` moves, rank `Master Adventurer`; export/import still pending | Z-machine DDEV draft package only; no `_demo` package yet | Package-local cover/small-cover/hero art and gameplay screenshots copied in DDEV; seven feelies copied in DDEV and classified as historical reference/preservation material, historical reference/preservation requiring caution, or likely commercial material requiring caution | Original how-to and progressive hints drafted; walkthrough route reaches the crown/score 175 on the selected artifact but remains unverified because the selected artifact has not been replaced. The scratch repaired-source artifact completes without `SUPERBRIEF` | Candidate only; DDEV package restored to draft | Decide whether to reopen the selected artifact basis for the `FANTASIES` repair, update package provenance if adopted, then run export/import smoke, final audit, and approval before any `_demo` work |
...
- Zork II's 2026-06-02 DDEV-only package uses the scratch-only `DREARY-ROOM-FCN` compatibility patch with historical release/serial reassembly; initial DDEV/Parchment routes, story checksum, helper-doc routes, selected-feelie routes, art/screenshot routes, release-specific metadata/iFiction, and material provenance checks passed. The normal-output post-crown balloon crash is now traced to a scratch-repairable `FANTASIES` table-counter issue, but the selected artifact was not replaced. The scratch repaired artifact now has a complete normal-output dfrotz transcript to `400/400` in `372` moves. Artifact-basis review, package provenance updates, export/import, audit, and approval remain before any `_demo` promotion.
