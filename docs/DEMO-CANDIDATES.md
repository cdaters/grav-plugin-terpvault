...
| [Zork III](demo-candidates/ZORK-III.md) | `https://github.com/historicalsource/zork3.git` rechecked at commit `3ec9ed412b5f3cafe65d83c727d07db1fe4a86a8` on `master`; no tags/releases observed | Strong candidate basis if MIT source release and redistribution requirements are confirmed; preserve upstream MIT license text and attribution; historical packaging/assets remain excluded unless separately licensed | Not ready for bundled demo; source build and `dfrotz` smoke were reverified in scratch on 2026-05-29, DDEV-only host-side route/story/Parchment boot checks passed for a temporary package, but final package metadata/assets/helper docs/audit are pending | Likely Z-machine story file package after package basis is resolved; see [candidate package plan](demo-candidates/ZORK-III-PACKAGE-PLAN.md) | Craig-created art, screenshots from the selected bundled playable version, and original package-local extras; see [asset/materials plan](demo-candidates/ZORK-III-ASSET-PLAN.md) | Original helper docs and feelies only unless upstream reusable docs are explicitly licensed; do not reuse `invisicluesiii.mss` without separate rights review | Candidate only; package/provenance/playback verification needed; legal/provenance review needed | Follow the candidate package and materials plans without creating `_demo` contents, then rerun final DDEV/Parchment playback/export/import checks once a complete candidate package exists |
...
- Zork III has stronger scratch build/playback evidence plus docs-only package and materials plans, but still needs complete package materials and final audit before bundled-demo review.
