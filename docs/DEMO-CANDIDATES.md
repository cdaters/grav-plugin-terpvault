...
- Zork II remains a candidate until build/package/playback verification and legal/provenance review are complete. Zork III has completed that path for this development demo tree and is bundled in `_demo`.
...
| [Zork III](demo-candidates/ZORK-III.md) | `https://github.com/historicalsource/zork3.git` rechecked at commit `3ec9ed412b5f3cafe65d83c727d07db1fe4a86a8` on `master`; no tags/releases observed | Approved for the bundled development demo tree from the verified MIT source release; preserve upstream MIT license text and attribution; historical packaging/assets remain excluded unless separately licensed | Source build and `dfrotz` smoke were reverified in scratch on 2026-05-29; DDEV package assembly, story-route diagnosis, complete-package export/import smoke, full walkthrough transcript verification, release-specific IFID/catalog metadata review, final audit, and final route/playback recheck passed on 2026-05-31; copied into `_demo/data/terpvault/games/zork-iii` after Craig approval | Bundled Z-machine starter package at `_demo/data/terpvault/games/zork-iii`; see [candidate package plan](demo-candidates/ZORK-III-PACKAGE-PLAN.md) | Craig-created/original art, screenshots, and package-local feelies were copied into the bundled `_demo` package; see [asset/materials plan](demo-candidates/ZORK-III-ASSET-PLAN.md) | Original helper docs were refreshed in the DDEV-only package on 2026-05-31 and copied into `_demo`; walkthrough is verified with `dfrotz -p -m -s 41` against the exact story, reaching 7 of 7 in 330 moves; IFID `ZCODE-25-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` are recorded; do not reuse `invisicluesiii.mss` without separate rights review | Approved for bundled demo; included in `_demo`; walkthrough fully verified for the bundled story | Keep package in `_demo`; run a clean demo-seed route/checksum recheck before release packaging |
