...
Terpwright Phase 3 should fit this same architecture as ecosystem lookup helpers, not remote package creation. Provider lookups should be optional, admin-triggered, timeout/rate-limit aware, and safe when offline. The review surface should compare current `game.yaml`, package-local iFiction XML, IFDB/IFWiki/IF Archive candidates, and source/license/provenance hints without treating remote catalog data as authoritative. Phase 3 must not add scraping, AI metadata generation, automatic story/art downloads, automatic publication, or hard runtime dependency on remote services.
