...
- Zork II should remain a candidate until approval and an explicit promotion pass, now that its DDEV-only patched source-built draft package has completed final audit after the repaired artifact adoption and export/import smoke. Zork III has completed that review path for this development demo tree and is bundled in `_demo`.
...
- Do not add Zork II to `_demo` until approval is explicit and a separate promotion pass copies only the reviewed DDEV package files and verifies checksums/routes after copy.
