...
- Zork II should remain a candidate until its DDEV-only patched source-built draft package resolves the selected artifact and walkthrough verification basis and has final Parchment playback coverage, package-level provenance, export/import smoke, and final audit complete. Zork III has completed that review path for this development demo tree and is bundled in `_demo`.
...
- Do not add Zork II to `_demo` until license/provenance, playback behavior, adopted artifact review, complete walkthrough verification, package-local original material review, historical reference/preservation material review, export/import audit, and final approval are complete.
