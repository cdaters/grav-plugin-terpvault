...
- Zork II should remain a candidate until its DDEV-only patched source-built draft package completes final audit and approval after the repaired artifact adoption and export/import smoke. Zork III has completed that review path for this development demo tree and is bundled in `_demo`.
...
- Zork II remains candidate-only. A DDEV-only draft package now uses the repaired patched source-built historical-header artifact `zork2.z3`; the DDEV story file has SHA-256 `02830587cfe5ca68c2f9289a9178780761ccec5f1582d13130d6217bd9e437ef`, Release 63 / Serial 860811, release-specific IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml`. Route/story/helper/selected-feelie/art/screenshot/dfrotz smoke checks passed, classified feelies, package-local art, gameplay screenshots, and verified helper docs are present. The adopted DDEV artifact includes the `DREARY-ROOM-FCN` build-compatibility patch and the `FANTASIES` table-counter repair, and the walkthrough verifies to `400/400` in `372` moves, rank `Master Adventurer`, without `SUPERBRIEF`. Export/import smoke also passed with a temporary non-overwriting draft import and imported-story dfrotz verification. The next Zork II work should run final audit while keeping conservative provenance notes for both source patches.
...
- Do not add Zork II to `_demo` until license/provenance, playback behavior, adopted artifact review, complete walkthrough verification, package-local original material review, historical reference/preservation material review, export/import audit evidence review, and final approval are complete.
