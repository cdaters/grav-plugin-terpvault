...
- Zork II remains candidate-only. A DDEV-only draft package now uses the documented patched source-built historical-header artifact `zork2-release63-serial860811.z3`; the DDEV story file has SHA-256 `10015c715e9226c491bbfe23e448df14e859a0d9f905afc4fe0c18d65d176019`, Release 63 / Serial 860811, release-specific IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml`. Route/story/helper/selected-feelie/art/screenshot/dfrotz smoke checks passed, classified feelies, package-local art, gameplay screenshots, and original draft helper docs are present. A scratch source/playback pass traced the normal-output post-crown balloon crash to a repairable `FANTASIES` table-counter issue, but the selected DDEV artifact has not been replaced and repaired-artifact walkthrough verification still fails after `Score: 348` / Cerberus from late-game Bank/gnome/demon payment ordering. The next Zork II work should decide whether to reopen the artifact basis for the `FANTASIES` repair, continue route cleanup, and keep conservative provenance notes for both scratch-only source patches.
...
- Do not add Zork II to `_demo` until license/provenance, playback behavior, selected artifact basis, complete walkthrough verification, package-local original material review, historical reference/preservation material review, export/import audit, and final approval are complete.
