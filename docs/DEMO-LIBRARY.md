...
Zork I is now bundled as the first reviewed starter/demo package in `_demo`. Zork II, Mainframe Zork / Dungeon, and Open Adventure / Colossal Cave remain candidates only; they require local build/package verification and final license/provenance review before bundling. Zork III has a final-audited DDEV-only draft candidate package and is ready for Craig approval, but it is not approved for `_demo` until that approval and a dedicated promotion pass happen. Mainframe Zork / Dungeon is research-only and should not get `_demo` package contents until it is explicitly approved beyond that status.
