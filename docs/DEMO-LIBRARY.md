...
- A v0.5.0 candidate demo suite may include Zork I as the verified anchor package. Zork II may join only if its DDEV-only patched source-built draft package reaches the same provenance/build/playback/helper-doc standard and passes walkthrough transcript verification, metadata/iFiction review, export/import, and final audit. Zork III has completed that path for the development `_demo` tree.
...
Zork I and Zork III are now bundled as reviewed starter/demo packages in `_demo`. Zork II, Mainframe Zork / Dungeon, and Open Adventure / Colossal Cave remain candidates only; they require local build/package verification and final license/provenance review before bundling. Zork II now has a DDEV-only draft package, classified feelies, draft helper docs, package-local art, gameplay screenshots, and initial playback/route verification, but it still needs walkthrough transcript verification, metadata/iFiction, export/import, audit, and approval before `_demo` promotion. Mainframe Zork / Dungeon is research-only and should not get `_demo` package contents until it is explicitly reviewed beyond that status.
