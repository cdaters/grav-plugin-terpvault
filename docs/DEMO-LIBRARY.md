...
| Zork III | `https://github.com/historicalsource/zork3.git` rechecked on `master` at commit `3ec9ed412b5f3cafe65d83c727d07db1fe4a86a8`; no tags/releases observed | Observed `LICENSE` is MIT text with `Copyright (c) 2025 Microsoft`, but packaging, trademarks, marketing materials, and non-source assets require final review | Scratch source build and `dfrotz` smoke were reverified on 2026-05-29; a real DDEV-only draft candidate package now exists at `/Users/cdaters/Sites/grav2.0-ddev/user/data/terpvault/games/zork-iii`; story-route diagnosis and complete-package export/import smoke passed on 2026-05-31 | Craig-created/original cover, small cover, hero, screenshots, and package-local feelies were copied into the DDEV-only candidate package; avoid historical commercial assets | Original helper docs were refreshed on 2026-05-31; walkthrough is now a polished draft based on local route references with partial `dfrotz` verification, but full transcript verification remains pending; do not reuse `invisicluesiii.mss` or other commercial helper material without separate rights review | Candidate only; full walkthrough/final audit required before redistribution |
