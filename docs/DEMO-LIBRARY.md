...
- A v0.5.0 candidate demo suite may include Zork I as the verified anchor package. Zork II may join only if its recommended patched source-built artifact basis completes DDEV/Parchment/package verification and reaches the same provenance/build/playback/helper-doc standard. Zork III has completed that path for the development `_demo` tree.
...
| Zork II | `https://github.com/historicalsource/zork2.git` rechecked on `master` at commit `3da9661098809788a99cef00f00c865c6c204f96`; no tags/releases observed | Observed `LICENSE` is MIT text with `Copyright (c) 2025 Microsoft`, but packaging, trademarks, marketing materials, and non-source assets require final review | Unmodified ZILF build still fails; 2026-06-02 artifact-basis recheck recommends the documented patched source-built historical-header artifact `zork2-release63-serial860811.z3` with SHA-256 `10015c715e9226c491bbfe23e448df14e859a0d9f905afc4fe0c18d65d176019`; upstream prebuilt `COMPILED/zork2.z3` also exists and smoke-tested, but is not the recommended default | Use original Craig-created artwork and package-local extras only; classify any selected historical reference/preservation materials under `docs/DEMO-CONTENT-RIGHTS.md` and avoid historical commercial assets unless item-level review supports inclusion | Original helper docs only unless a reusable source has documented reuse rights | Candidate only; DDEV/Parchment package verification required before redistribution |
...
Zork I and Zork III are now bundled as reviewed starter/demo packages in `_demo`. Zork II, Mainframe Zork / Dungeon, and Open Adventure / Colossal Cave remain candidates only; they require local build/package verification and final license/provenance review before bundling. Zork II has a recommended next artifact basis, but no package should be created until DDEV/Parchment playback and package-local provenance are verified. Mainframe Zork / Dungeon is research-only and should not get `_demo` package contents until it is explicitly reviewed beyond that status.
