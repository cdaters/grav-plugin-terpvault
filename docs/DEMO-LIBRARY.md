...
- A v0.5.0 candidate demo suite may include Zork I as the verified anchor package. Zork II may join only if its DDEV-only patched source-built draft package reaches the same provenance/build/playback/helper-doc standard, resolves the selected artifact and walkthrough verification basis, and passes export/import and final audit. Zork III has completed that path for the development `_demo` tree.
...
| Zork II | `https://github.com/historicalsource/zork2.git` rechecked on `master` at commit `3da9661098809788a99cef00f00c865c6c204f96`; no tags/releases observed | Observed `LICENSE` is MIT text with `Copyright (c) 2025 Microsoft`; trademarks and supplemental materials remain separately classified under `docs/DEMO-CONTENT-RIGHTS.md` | DDEV-only draft package assembled from the documented patched source-built historical-header artifact; `zork2.z3` SHA-256 `10015c715e9226c491bbfe23e448df14e859a0d9f905afc4fe0c18d65d176019`, 92412 bytes, Release 63 / Serial 860811; release-specific IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` are recorded; temporary published detail/play/story/helper/selected-feelie/art/screenshot routes and dfrotz smoke passed; metadata manifest warnings are now clear; selected artifact still reproduces the normal-output post-crown balloon crash, while a scratch repaired-source artifact with a `FANTASIES` table-counter fix completes under normal dfrotz output to `400/400` in `372` moves, rank `Master Adventurer`; export/import and final audit remain pending | Package-local cover, small cover, hero art, and gameplay screenshots are present in DDEV; seven DDEV feelies are classified as historical reference/preservation material, historical reference/preservation requiring caution, or likely commercial material requiring caution | Original how-to and progressive hints drafted; walkthrough route remains unverified for the selected DDEV artifact because it has not been replaced. Scratch repaired-source evidence now has a complete transcript without `SUPERBRIEF` | Candidate only; DDEV package restored to draft, not in `_demo` |
