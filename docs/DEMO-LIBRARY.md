...
| Zork III | `https://github.com/historicalsource/zork3.git` rechecked on `master` at commit `3ec9ed412b5f3cafe65d83c727d07db1fe4a86a8`; no tags/releases observed | Observed `LICENSE` is MIT text with `Copyright (c) 2025 Microsoft`, but packaging, trademarks, marketing materials, and non-source assets require final review | Scratch source build and `dfrotz` smoke were reverified on 2026-05-29; a real DDEV-only draft candidate package now exists at `/Users/cdaters/Sites/grav2.0-ddev/user/data/terpvault/games/zork-iii`; story-route diagnosis, complete-package export/import smoke, and full walkthrough transcript verification passed on 2026-05-31 | Craig-created/original cover, small cover, hero, screenshots, and package-local feelies were copied into the DDEV-only candidate package; avoid historical commercial assets | Original helper docs were refreshed on 2026-05-31; walkthrough is verified with `dfrotz -p -m -s 41` against the exact package story, reaching 7 of 7 in 330 moves; local IFID review did not find an authoritative IFID because no Treaty/Babel extractor was installed and no IFID string was found in the story file; do not reuse `invisicluesiii.mss` or other commercial helper material without separate rights review | Candidate only; final audit, authoritative IFID/catalog/iFiction resolution, route/playback recheck, and approval required before redistribution |
...
Zork I is now bundled as the first reviewed starter/demo package in `_demo`. Zork II, Zork III, Mainframe Zork / Dungeon, and Open Adventure / Colossal Cave remain candidates only; they require local build/package verification and final license/provenance review before bundling. Zork III has a DDEV-only draft candidate package, but it is not approved for `_demo`. Mainframe Zork / Dungeon is research-only and should not get `_demo` package contents until it is explicitly approved beyond that status.
