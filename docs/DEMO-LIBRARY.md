...
- A v0.5.0 candidate demo suite may include Zork I as the verified anchor package. Zork II may join only if its DDEV-only patched source-built draft package reaches the same provenance/build/playback/helper-doc standard and completes final audit and approval after the verified artifact adoption and export/import smoke. Zork III has completed that path for the development `_demo` tree.
