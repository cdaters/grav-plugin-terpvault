...
| Open Adventure / Colossal Cave | Open Adventure project/release source at https://gitlab.com/esr/open-adventure (`master` commit `993291a21da44234ae9cf303d0ffc0df19ec3c31`) | Review in progress: BSD 2-Clause + CC-BY-4.0 project metadata scope must be validated per asset/document type before any final release | Draft package uses an Inform 6 / Z-machine `.z8` port; story SHA-256 `284f3d08ab9cc692457d1a982acf7dddffcaf8bf0b4aa71cb5d1f33d82865d43`; IFID `74146740-24EA-5383-A8BF-8B239CE36DBE`; native `advent`/`cheat` build remains the caveated oracle | Draft `_demo` package at `_demo/data/terpvault/games/open-adventure`; `terpvault.status: draft`; `terpvault.featured: false`; not final/release-ready | Package-local cover, small cover, hero, screenshots, and a draft map/mockup feelie are included for evaluation; final map/poster art remains replaceable | Original helper docs, walkthrough, provenance, known differences, history/details, map notes, and treasure checklist are included | Draft demo/library candidate; verify before redistribution |
...
Zork I, Zork II, and Zork III are now bundled as reviewed starter/demo packages in `_demo`. Open Adventure / Colossal Cave is present in `_demo` as a draft, non-featured demo/library candidate only; it requires final art/provenance review and route/checksum validation before any publish decision. Mainframe Zork / Dungeon remains research-only and should not get `_demo` package contents until explicitly reviewed beyond that status.
