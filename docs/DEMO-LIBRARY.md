...
- A v0.5.0 candidate demo suite may include Zork I as the verified anchor package. Zork II may join only after approval and an explicit promotion pass now that its DDEV-only patched source-built draft package has reached the same provenance/build/playback/helper-doc standard and completed final audit after the verified artifact adoption and export/import smoke. Zork III has completed that path for the development `_demo` tree.
...
| Zork II | `https://github.com/historicalsource/zork2.git` rechecked on `master` at commit `3da9661098809788a99cef00f00c865c6c204f96`; no tags/releases observed | Observed `LICENSE` is MIT text with `Copyright (c) 2025 Microsoft`; trademarks and supplemental materials remain separately classified under `docs/DEMO-CONTENT-RIGHTS.md` | DDEV-only draft package now uses the repaired patched source-built historical-header artifact; `zork2.z3` SHA-256 `02830587cfe5ca68c2f9289a9178780761ccec5f1582d13130d6217bd9e437ef`, 92414 bytes, Release 63 / Serial 860811; release-specific IFID `ZCODE-63-860811`, IFDB TUID/URL, IFWiki URL, and package-local `metadata.iFiction.xml` are recorded; adopted source patches are `DREARY-ROOM-FCN` build compatibility and `FANTASIES` table-counter repair; temporary published detail/play/story/helper/art routes and manifest checks passed; dfrotz verification completed to `400/400` in `372` moves, rank `Master Adventurer`; export/import smoke, temporary import routes, imported-story dfrotz verification, and final package audit passed | Package-local cover, small cover, hero art, and gameplay screenshots are present in DDEV; seven DDEV feelies are classified as historical reference/preservation material, historical reference/preservation requiring caution, or likely commercial material requiring caution | Original how-to, progressive hints, and verified walkthrough are present in DDEV; walkthrough completes without `SUPERBRIEF` | Candidate only; DDEV package restored to draft, not in `_demo`; ready for approval/promotion decision |
...
Zork I and Zork III are now bundled as reviewed starter/demo packages in `_demo`. Zork II, Mainframe Zork / Dungeon, and Open Adventure / Colossal Cave remain candidates only; they require local build/package verification and final license/provenance review before bundling. Zork II now has a DDEV-only draft package with the repaired verified artifact adopted, classified feelies, verified helper docs, package-local art, gameplay screenshots, release-specific IFID/catalog/iFiction metadata, playback/route verification to `400/400` in `372` moves, export/import smoke coverage, and a passed final audit. It still needs approval and an explicit promotion pass before `_demo` promotion. Mainframe Zork / Dungeon is research-only and should not get `_demo` package contents until it is explicitly reviewed beyond that status.
