...
- `zork-ii`: candidate only. A DDEV-only draft package was assembled on 2026-06-02 from the documented patched source-built historical-header artifact; route/story/helper-doc/selected-feelie/art/screenshot checks and dfrotz smoke passed, classified feelies, package-local art, gameplay screenshots, and original draft helper docs are present, and the package was restored to draft. Walkthrough transcript verification, IFID/catalog/iFiction metadata, export/import smoke, and package audit must be complete before bundling.
