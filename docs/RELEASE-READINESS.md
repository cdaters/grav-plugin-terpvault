...
- Demo package posture is credible: Zork I and Zork III are fully bundled and verified; `docs/DEMO-CONTENT-RIGHTS.md` is followed; Zork II is added only after explicit approval and a separate promotion pass now that its DDEV-only patched source-built draft package has completed final audit after the repaired verified artifact adoption and export/import smoke; Adventure / Colossal Cave, Grue, and You Are Standing are polished only if story-file licensing and package-level provenance are complete for the intended distribution.
...
- `zork-ii`: candidate only. A DDEV-only draft package was assembled on 2026-06-02 and now uses the repaired patched source-built historical-header artifact; route/story/helper-doc/selected-feelie/art/screenshot checks and dfrotz smoke passed, classified feelies, package-local art, gameplay screenshots, verified helper docs, release-specific IFID `ZCODE-63-860811`, IFDB/IFWiki catalog fields, and package-local `metadata.iFiction.xml` are present, and the package was restored to draft. The adopted artifact has SHA-256 `02830587cfe5ca68c2f9289a9178780761ccec5f1582d13130d6217bd9e437ef`, includes the `DREARY-ROOM-FCN` build-compatibility patch and `FANTASIES` table-counter repair, and completes under normal dfrotz output to `400/400` in `372` moves, rank `Master Adventurer`. Export/import smoke passed with a temporary non-overwriting draft import, clean routes, and imported-story dfrotz verification. Final package audit passed on 2026-06-02. Approval and an explicit promotion pass must be complete before bundling.
