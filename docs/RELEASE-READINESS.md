...
- `zork-ii`: candidate only. A DDEV-only draft package was assembled on 2026-06-02 from the documented patched source-built historical-header artifact; route/story/helper-doc/selected-feelie/art/screenshot checks and dfrotz smoke passed, classified feelies, package-local art, gameplay screenshots, original draft helper docs, release-specific IFID `ZCODE-63-860811`, IFDB/IFWiki catalog fields, and package-local `metadata.iFiction.xml` are present, and the package was restored to draft. The selected artifact still reproduces the normal-output post-crown balloon crash, but a scratch repaired-source artifact with a `FANTASIES` table-counter fix gets past that descent and reaches `Score: 348` / Cerberus before blocking on late-game Bank/gnome/demon payment ordering. The selected artifact basis, full route verification, export/import smoke, and package audit must be complete before bundling.
