...
- Demo package posture is credible: Zork I and Zork III are fully bundled and verified; `docs/DEMO-CONTENT-RIGHTS.md` is followed; Zork II is added only if its recommended patched source-built artifact basis, provenance, playback, package audit, and helper docs are complete; Adventure / Colossal Cave, Grue, and You Are Standing are polished only if story-file licensing and package-level provenance are complete for the intended distribution.
...
- `zork-ii`: candidate only. The 2026-06-02 artifact-basis recheck recommends the documented patched source-built historical-header artifact as the next package-basis candidate, but DDEV/Parchment playback, provenance, helper docs, package-local original materials, any historical reference/preservation materials, and package audit must be complete before bundling.
