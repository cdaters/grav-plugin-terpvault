...
- Demo package posture is credible: Zork I and Zork III are fully bundled and verified; `docs/DEMO-CONTENT-RIGHTS.md` is followed; Zork II is added only if its DDEV-only patched source-built draft package completes final audit and approval after the repaired verified artifact adoption and export/import smoke; Adventure / Colossal Cave, Grue, and You Are Standing are polished only if story-file licensing and package-level provenance are complete for the intended distribution.
