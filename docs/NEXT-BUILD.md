...
- Use `docs/TERPWRIGHT.md` as the Terpwright planning baseline. The realistic near-term path is Phase 1 local-file package building: local story file or inspected package zip, local assets/helper docs/iFiction XML, manual ecosystem references, validation report, draft-only install, and `.terpvault.zip` export. Remote lookup/search, metadata assistant providers, cover generation/art suggestions, automated provenance drafting, richer Oracle generation, and guided workflows remain later phases.
