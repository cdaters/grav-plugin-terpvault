...
- Continue demo package preparation incrementally. Zork I and Zork III are the stronger bundled/demo anchors for the current development tree. Zork II remains candidate-only until export/import smoke, final audit, and approval are resolved for the repaired verified DDEV artifact.
...
- Return to Zork II candidate work with export/import smoke and final audit now that the repaired artifact has been adopted into the DDEV package and verifies under normal dfrotz output to `400/400` in `372` moves. Keep reviewing both source patches in provenance: `DREARY-ROOM-FCN` for ZILF compatibility and the `FANTASIES` table-counter repair for the normal-output balloon descent crash. Do not make a `_demo` decision until export/import, audit, and approval are complete.
