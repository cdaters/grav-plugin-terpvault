...
- Remote IFWiki, IF Archive file download, or broad catalog lookup/import is not implemented.
- Terpwright Phase 3a/3b ecosystem preview is implemented as a read-only Admin2 helper with IF Archive URL/path normalization; Phase 3c adds IFDB TUID/URL normalization and read-only IFDB metadata preview through IFDB's official API.
...
- Metadata/iFiction workflows should be clear: local `metadata.iFiction.xml` preview/apply documented, IF Archive path/URL normalization documented, IFDB preview documented as reference-only, and IFWiki/IF Archive file lookup still clearly roadmap unless explicitly implemented.
...
- Continue the preview-driven Metadata Assistant and Terpwright Phase 3 ecosystem lookup helpers around local `game.yaml`, package-local or uploaded `metadata.iFiction.xml`, current IF Archive path/URL normalization, current IFDB preview, and later explicit IFWiki lookup. It must never silently overwrite metadata, must back up `game.yaml`, and must keep license/provenance review distinct from story-file/package download.
...
- Terpwright Phase 1 local-file package building, Phase 2 manual metadata/provenance URL capture, Phase 3a/3b ecosystem preview with IF Archive URL/path normalization, and Phase 3c IFDB metadata preview are now implemented for draft-oriented Admin2 workflows. Keep package-zip inspection/import on the existing import path. Future Phase 3 work should continue with IFWiki helper, iFiction/Babel cross-check, richer curator apply/diff workflow, and validation integration.
