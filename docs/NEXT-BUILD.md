...
- Terpwright Phase 1 local-file package building is now implemented for new draft package creation from a local story file plus optional local assets/helper docs/iFiction XML. Keep package-zip inspection/import on the existing import path. Remote lookup/search, metadata assistant providers, cover generation/art suggestions, automated provenance drafting, richer Oracle generation, and guided workflows remain later phases.
