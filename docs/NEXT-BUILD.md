...
- Continue demo package preparation incrementally. Zork I and Zork III are the stronger bundled/demo anchors for the current development tree. Zork II remains candidate-only until approval and an explicit promotion pass, even though final audit has passed for the repaired verified DDEV artifact after export/import smoke.
...
- Return to Zork II candidate work with an approval decision and explicit promotion pass if approved. The repaired artifact has been adopted into the DDEV package, verifies under normal dfrotz output to `400/400` in `372` moves, has passed export/import smoke with a temporary non-overwriting draft import, and passed final package audit. Keep reviewing both source patches in provenance: `DREARY-ROOM-FCN` for ZILF compatibility and the `FANTASIES` table-counter repair for the normal-output balloon descent crash. Do not make a `_demo` copy until approval is explicit.
