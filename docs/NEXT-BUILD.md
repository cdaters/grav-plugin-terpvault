...
- Polish public/demo suite candidates such as Open Adventure / Colossal Cave, Grue, and You Are Standing only when story files, art, helper docs, and license/provenance notes are original or properly licensed. Open Adventure currently has a verified native build and baseline tests in-lab, but the result is a caveated oracle (`115 tests, 2 failures`: `saveresume.1`, `saveresume.3`) rather than a zero-failure baseline. TerpVault cannot play native `advent` directly; the preferred future playable target is a Z-machine `.z8` port, likely via Inform 6 or another documented Z-machine path. No TerpVault package or `_demo` content exists yet.
