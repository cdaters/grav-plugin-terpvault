...
- Continue demo package preparation incrementally. Zork I and Zork III are the stronger bundled/demo anchors for the current development tree. Zork II remains candidate-only until final audit and approval are resolved for the repaired verified DDEV artifact after export/import smoke.
...
- Return to Zork II candidate work with final package audit now that the repaired artifact has been adopted into the DDEV package, verifies under normal dfrotz output to `400/400` in `372` moves, and has passed export/import smoke with a temporary non-overwriting draft import. Keep reviewing both source patches in provenance: `DREARY-ROOM-FCN` for ZILF compatibility and the `FANTASIES` table-counter repair for the normal-output balloon descent crash. Do not make a `_demo` decision until audit and approval are complete.
